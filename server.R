...
source("~/mtcarsdeclarado.R")
source("~/FuncoesTabelas.R")
source("~/nhanesdeclarado.R")
...
shinyServer(function(input, output) {
  ...
  ############################################################################################################
  ############################################################################################################
  output$selecaovari <- renderUI({
    nomes <- names(dtset())
    selectInput("varitabe", "Escolha a variável:", nomes)
  })
  
  funcao <- reactive({
    switch(input$func,
           "nominal" = TabelaNominal,
           "ordinal" = TabelaOrdinal,
           "quantitativa" = TabelaQuantitativa,
           "simples" = TabelaFrequenciaSimples
    )
  })
  
  output$tabelacombanco <- renderTable({
    auxiliar <- dtset()[,input$varitabe]
    doit <- funcao()
    doit(auxiliar)
  },rownames = T)
