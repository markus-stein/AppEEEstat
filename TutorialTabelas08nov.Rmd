---
title: "TutorialTabelasQualiQuanti"
author: "Arthur Rasbold Neves"
...
Tabelaux <- function(variavel){
  ...
  tabelaux <- cbind(tabelaux, tabelaux/length(variavel))
...
# #  Função Tabelaauxord
# 
# Tavelauxord <- function(variavel){
#   tabelauxord <- TabelaNominal(variavel)
#   tabelauxord <- tabelauxord[-nrow(tabelauxord),]
#   tabelauxord <- cbind(tabelauxord, cumsum(table(variavel)))
#   tabelauxord <- cbind(tabelauxord, cumsum((table(variavel)/length(variavel))))
#   colnames(tabelauxord)  <-  c("f", "fr", "F", "Fr")
#   return(tabelauxord)
# }
# 
# #  Função Totaux
# 
# Totaux <- function(variavel){
#   totaux <- c(sum(table(variavel)), sum(table(variavel))/(length(variavel)))
#   return(totaux)
# }
...
numcatcarb
```
 A variável "carb" tem 6 categorias então é preferível que a tabela para "carb" não apresente intervalos de classe. Devemos declarar "carb"" como se fosse uma variável qualitativa ordinal.
```{r}
...
```{r Criação de 4 tabelas, exercise = TRUE}
